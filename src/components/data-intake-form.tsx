...
function DataIntakeForm({ userId, categories, attributes }: DataIntakeFormProps) {
  const [currentResponses, setCurrentResponses] = useState<Set<string>>(new Set());
  ...
  const attributesByCategory = useMemo(() => {
    ...
    if (attributes.length && categories) {
      categories.forEach(category => {
        map.set(category.id, attributes.filter(attr => attr.category_id === category.id));
      });
    }
    return map;
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    setScaleError(false);
    const removedResponses = new Set([...currentResponses].filter(x => !currentSelection.has(x)));
    const addedResponses = new Set([...currentSelection].filter(x => !currentResponses.has(x)));
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-2">
      <div className="bg-white/50 backdrop-blur-sm mt-4 mx-4 rounded-full mb-6 py-4 px-2">
        <div className="container mx-auto px-4 py-4 h-16 flex items-center justify-between">
          <div className="flex flex-col items-left pl-2">
            <p className="text-2xl font-semibold text-black">Daily Habit Form</p>
            {completedForm ? (
              ...
              <p className="text-center text-sm text-gray-500">
                You have yet to complete your habit form for the day. Place your selections and submit.
              </p>
            )}
          </div>
      ...
      <div className="relative">
        ...
        {/* Scale Selection */}
        <div className={`flex flex-col items-center py-4 bg-white/50 rounded-full mb-6 z-10 transition-opacity ${submitting ? "opacity-50" : "opacity-100"}`}>
          ...
          {scaleError && <p className="text-red-500 mt-2 text-sm">
            ...
            return (
              <Card
                ...
                className={`bg-white/50 break-inside-avoid backdrop-blur-sm rounded-2xl border-none relative transition-opacity ${
                  submitting || !scaleSelection ? "opacity-50 pointer-events-none" : "opacity-100"
                }`}
              >
